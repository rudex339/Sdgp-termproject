...
public class Enemy2 extends Enemy{
    ...
    private Enemy2(int level, int index) {
        super(level, index);
        init(level, index);
        dx = -SPEED;
    }
    ...
    private void init(int level, int index) {
        ...
        if(srcRect == null)
            srcRect=new Rect(  160, 43,   160 + 17, 43+17);
        else srcRect.set(160, 43,   160 + 17, 43+17);

        setPosition(16.f/3 + 1.f * 7, 3.f+1.f*(index), 0.5f);
        //setPosition(16.f/3 + 1.f * 7, 3.f+1.f*(index), 1.f);
    }

    public static Enemy2 get(int level, int index) {
        Enemy2 enemy = (Enemy2) RecycleBin.get(Enemy2.class);
        if (enemy != null) {
            enemy.init(level, index);
            return enemy;
        }
        return new Enemy2(level, index);
    }
    @Override
